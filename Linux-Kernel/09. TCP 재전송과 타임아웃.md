	...
	- **재전송 횟수 계산 (`tcp_orphan_retries()` 함수):**
	    ...
	    - **alive 로직:** 소켓의 현재 RTO 값이 `TCP_RTO_MAX`(120초)보다 작으면 `alive`는 **1(True)** 이 된다. 
		    - ```c
				...
				```
	    - **강제 재설정 (8회의 법칙):** 파라미터 설정을 **0**으로 하더라도, 소켓이 `alive` 상태라면 커널은 내부적으로 재전송 횟수를 **8회**로 변경하여 반환한다. 
		    - ```c
...
- **Connection Timeout 설정 전략:** 첫 SYN 패킷에 대한 재전송은 1초 후, 그다음은 2초 후에 일어난다. 따라서 최소 **3초**로 설정해야 두 번의 재전송 기회를 가질 수 있다. 
