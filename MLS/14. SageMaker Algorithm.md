...
- **용도**: 
	...
	- 주제 자체는 레이블이 지정되지 않으며, 단지 공통의 단어 하위 집합을 공유하는 문서들의 그룹일 뿐입니다. -> 문서 내 단어 패턴 기반 그룹화
...
- **하이퍼파라미터**:
    ...
    - `alpha0`: 집중도 매개변수(concentration parameter) (작을수록 희소한 토픽, 클수록 균등한 토픽)
...
- **용도**: 분류(Classification)와 회귀(Regression).
    
    - 분류: 가까운 K개 데이터 중 다수 라벨.
        
    - 회귀: 가까운 K개 데이터의 평균 값.
        
- **입력**: RecordIO-Protobuf 또는 CSV (첫 열은 라벨).
    
- **활용**: 차원 축소 기법(sign, fjlt) 적용 후 최근접 이웃 탐색.
    
- **하이퍼파라미터**: `k`, `sample_size`.
    
- **인스턴스**: CPU/GPU 모두 지원 (m5.2xlarge, p2.xlarge).
...
- **입력**: RecordIO-Protobuf 또는 CSV (train/test).
    
- **알고리즘**: 초기 클러스터(k-means++), 반복 계산 후 최적 클러스터 중심 탐색.
    
- **하이퍼파라미터**:
    
    - `k`: 클러스터 수 (Elbow Method 사용).
        
    - `mini_batch_size`, `extra_center_factor`, `init_method`.
        
- **인스턴스**: CPU 권장, GPU 사용 시 1개만 지원 (ml.g4dn.xlarge).
...
- **용도**: 차원 축소 (고차원 데이터를 저차원으로 투영).
    
- **입력**: RecordIO-Protobuf 또는 CSV.
    
- **알고리즘**: 공분산 행렬 생성 → 특잇값 분해(SVD).
    
    - Regular 모드: 희소 데이터, 중간 규모.
        
    - Randomized 모드: 대규모 데이터.
        
- **하이퍼파라미터**: `algorithm_mode`, `subtract_mean`.
...
- **용도**: 희소 데이터 기반 예측 (클릭 예측, 추천 시스템).
    
- **학습 유형**: 지도학습 (분류·회귀).
    
- **입력**: RecordIO-Protobuf (float32).
